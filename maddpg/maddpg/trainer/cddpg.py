...
def p_train(make_obs_ph_n, act_space_n, p_func, q_func, optimizer, grad_norm_clipping=None, local_q_func=False, num_units=64, scope="trainer", reuse=None):
...
def c_train(make_obs_ph_n, act_space_n, con_space_n, optimizer, grad_norm_clipping=None, local_q_func=False, scope="trainer", reuse=None, num_units=64):
    return con, update_target_con, {'c_values': c_values, 'target_c_values': target_c_values}
class CDDPGAgentTrainer(AgentTrainer):
    def __init__(self, model, obs_shape_n, act_space_n, con_space_n, agent_num, args, local_q_func=False):
        ...
        self.c_train, self.c_update, self.c_debug = c_train(
            scope="ddpg",
            make_obs_ph_n=obs_ph_n,
            act_space_n=act_space_n,
            con_space_n=con_space_n,
            optimizer=tf.train.AdamOptimizer(learning_rate=args.lr),
            grad_norm_clipping=0.5,
            local_q_func=local_q_func,
            num_units=args.num_units
    ...
    def update(self, trainer, t):
        ...
        obs, act, con, rew, obs_next, done = trainer[0].replay_buffer.sample_index(index)
        obs_n.append(obs)
