...
class World(object):
    ...
    def get_collision_force(self, entity_a, entity_b):
        ...
        if (entity_a.state.p_pos[0] - entity_b.state.p_pos[0]) != 0:
            ang = np.arctan((entity_a.state.p_pos[1] - entity_b.state.p_pos[1])/(entity_a.state.p_pos[0] - entity_b.state.p_pos[0]))
        else:
            ang = np.pi/2
        dist = np.sqrt(np.sum(np.square(delta_pos)))
        # minimum allowable distance
        dist_min = (entity_a.size + entity_b.size)*np.cos(abs(ang)-np.pi/4)
        # softmax penetration
