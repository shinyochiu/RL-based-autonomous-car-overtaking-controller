...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        for i, dynamic_obs in enumerate(world.dynamic_obs):
            dynamic_obs.name = 'static_obs %d' % i
        ...
        for i, obstacle in enumerate(world.dynamic_obs):
            obstacle.state.p_pos = world.start
            obstacle.state.p_ang = np.arctan2(world.path[1][1] - obstacle.state.p_pos[1], world.path[1][0] - obstacle.state.p_pos[0])
            obstacle.state.p_vel = 0.25*np.array(world.path[1] - world.start)/np.linalg.norm(world.path[1] - world.start)
        ...
        for agent in world.agents:
            ...
            #agent.state.p_vel = np.array(world.goal-world.start)
        ...
        world.goal_vect = 7*np.linalg.norm(world.dynamic_obs[0].state.p_vel)*(world.dynamic_obs[0].state.p_pos - world.agents[0].state.p_pos)/np.linalg.norm(world.dynamic_obs[0].state.p_pos - world.agents[0].state.p_pos)
        world.goal = world.dynamic_obs[0].state.p_pos + world.goal_vect
    ...
    def agent_reward(self, agent, world):
        ...
        if agent.collide:
            ...
            for o in static_obs:
                ...
                ang2obs[ang2obs.index(max(ang2obs))] = np.arccos(np.clip(np.dot((o.state.p_pos - a.state.p_pos), agent.state.p_vel)/(np.linalg.norm(agent.state.p_vel)*np.linalg.norm(o.state.p_pos - a.state.p_pos)), -1, 1))
            ...
            for o in dynamic_obs:
                ...
                ang2leader[ang2leader.index(max(ang2leader))] = np.arccos(np.clip(np.dot((o.state.p_pos - a.state.p_pos), agent.state.p_vel)/(np.linalg.norm(agent.state.p_vel)*np.linalg.norm(o.state.p_pos - a.state.p_pos)), -1, 1))

            if abs(min(ang2leader)) < np.pi/6:
                rew -= alpha*np.exp(-beta * min(dis2leader))
            elif abs(min(ang2leader)) < 2*np.pi/3:
                rew -= alpha*np.exp(-beta / (abs(0.66-min(dis2leader))*abs(np.sin(min(ang2leader)))))
            else:
                rew -= alpha*np.exp(-beta / (abs(agent.ang2goal - agent.state.p_ang)+1e-6))
    ...
    def observation(self, agent, world):
        ...
        formation_pos_x = []
        formation_pos_y = []
        formation_pos_x.append(agent.state.p_pos[0])
        formation_pos_y.append(agent.state.p_pos[1])
        #print(formation_pos_x)
        comm = [world.agents[0].state.c]

        leader = world.agents[0]
        if not agent.leader:
            '''other_pos.append(agent.state.p_pos - leader.state.p_pos)
            other_dis.append(np.array([np.linalg.norm(agent.state.p_pos - leader.state.p_pos)]))
            other_vel.append(agent.state.p_vel)
            other_ang.append(np.array([agent.state.p_ang]))'''
            for other in world.agents:
                if other is agent: continue
            ...
            for other in world.agents:
                ...
                comm.append(other.state.c)
                other_pos.append(other.state.p_pos - agent.state.p_pos)
                other_dis.append(np.array([np.linalg.norm(other.state.p_pos - agent.state.p_pos)]))
                if not other.adversary:
                    other_vel.append(other.state.p_vel)
                    other_ang.append(np.array([agent.state.p_ang]))
                formation_pos_x.append(other.state.p_pos[0])
        ...
        if agent.leader:
            ...
            #print(dis2goal + ang_err + entity_dis + entity_ang)
    ...
    def done(self, agent, world):
        ...
        if dis < 1e-1:
            ...
            '''if world.station == world.station_num:
                ...
                world.station += 1
                world.goal = world.path[world.station-1]'''
        else:
            if world.station < world.station_num:
                world.goal = world.dynamic_obs[0].state.p_pos + world.goal_vect
        return False
